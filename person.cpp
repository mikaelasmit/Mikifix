...
person::person()											// First 'person' class second constructor/variable and no return type means its a constructor
{
    ...
    HPV_Status=-999;
    HPV_StatusAtRecovery=-999;
    HPV_DateofInfection=-999;
    HPV_DateofRe_Infection=-999;
...
void person::GetMyDateOfHPVInfection(){
    ...
    if(age_at_death>=age_atrisk_hpv && Sex==2){
        ...
        if (h>(HPVarray[2][65])*HPV_Array_buffer){HPV_DateofInfection=-988;}// They do NOT get HPV ever
        
        if (h<=(HPVarray[2][65])*HPV_Array_buffer){                         // They will get HPV in their life
            int i=0;
